...
async function sendPasswordResetEmail({ email, name, token }, config = process.env) {
  ...
  const logoPath = path.resolve(__dirname, '../../frontend/public/assets/tatui-imoveis-logo-light.svg');
  const logoSvg = fs.readFileSync(logoPath, 'utf8').replace('<svg ', '<svg width="240" height="57" style="display:block;margin:0 auto" ');
