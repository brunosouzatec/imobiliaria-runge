...
function PropertyCard({ item }) { return <a className="react-property-card" href={`imovel.html?id=${item.id}`}><span>{item.tipo} · {item.categoria}</span><h2>{item.titulo}</h2><p>{item.endereco}</p><strong>{money(item.preco, item.tipo)}</strong><b>Ver detalhes →</b></a>; }
...
const Page = path.endsWith('login.html') ? LoginPage : path.endsWith('imovel.html') ? DetailPage : path.endsWith('sucesso.html') ? SuccessPage : path.endsWith('perfil.html') ? ProfilePage : ListPage;
